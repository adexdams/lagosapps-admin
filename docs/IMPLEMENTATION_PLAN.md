...
- **Welcome email trigger** — fires automatically on every new `profiles` row via `pg_net` + Edge Function (covers both user-app signup and admin-created users); **confirmed live 2026-04-25 — user signed up via user app, welcome email arrived from `hello@lagosapps.com`**
...
- **Admin Orders / Live Carts wired to DB** — Orders list (with SLA stat cards: Total, Pending, At Risk, Behind SLA) + Order Detail (status change, refund-to-wallet, fulfillment tracking with assignee + internal notes, risk level auto-computed from platform SLA settings), Create Order admin wizard (debounced customer search, multi-portal product picker, full-screen layout), Live Carts (real `carts` + `cart_items`, abandoned detection, Send Reminder via Resend). Fulfillment nav item removed; risk level derived from `sla_hours` + `sla_warning_hours` in Platform Settings. **M3 fully tested 2026-04-26** — bugs resolved: refund txn ID varchar(20) overflow, single-portal order restriction removed, duplicate status dropdowns consolidated, internal notes author_id race condition fixed.
