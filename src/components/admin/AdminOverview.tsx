...
export default function AdminOverview() {
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      {/* Admin welcome card */}
      <div className="bg-white rounded-2xl shadow-[0_1px_3px_rgba(0,0,0,0.04)] border border-[#E8ECF1]/60 p-4 sm:p-6 flex items-center gap-4">
        <div className="size-12 sm:size-14 rounded-full bg-gradient-to-br from-primary to-primary/70 flex items-center justify-center text-white font-bold text-base sm:text-lg shadow-sm flex-shrink-0">
          {initials}
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-base sm:text-lg font-bold text-[#0F172A]">Welcome back, {user?.name?.split(" ")[0] || "Admin"}</p>
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1">
            ...
            <span className="text-sm text-[#64748B] flex items-center gap-1">
              ...
              {user?.whatsappNumber || "No WhatsApp set"}
            </span>
            <span className="text-xs font-semibold text-primary bg-primary/8 px-2 py-0.5 rounded-full">
              {user?.role?.replace("_", " ").replace(/\b\w/g, (c) => c.toUpperCase()) || "Admin"}
            </span>
          </div>
        </div>
      </div>
